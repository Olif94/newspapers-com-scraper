    ...
    return new Promise((resolve, reject) => {
        ...
        scraper.on('article', (article) => {
            ...
            articles.push({
                ...
                // Keyword metrics
                keyword: "chinese AND cheap labor",
...
async function main() {
    ...
    for (const [index, quarter] of quarterlyChunks.entries()) {
        ...
        if (articles.length > 0) {
            ...
            console.log(`   💾 Saved: Cheap_${quarter.name}_comprehensive.csv (${articles.length} articles)`);
        }
    ...
    if (allArticles.length > 0) {
        ...
        console.log(`\\n💾 MASTER DATASET: Cheap_ID_1870_1874_comprehensive_${allArticles.length}_articles.csv`);
...
async function generateSummaries(articles) {
    ...
    const newspaperWriter = createCsvWriter({
        path: `Cheap_newspaper_summary_1870_1874.csv`,
        header: [
            {id: 'newspaper_title', title: 'Newspaper_Title'},
            {id: 'city', title: 'City'},
            {id: 'county', title: 'County'},
    ...
    console.log(`💾 Newspaper summary: Cheap_newspaper_summary_1870_1874.csv`);
